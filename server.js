...
function localExplanation(q) {
  ...
  if (op === 'iq') {
    ...
    if (q.type === 'numerical') { const val = q.options?.[q.answer] ?? String(answer); return {title:'Penalaran numerik',summary:'Tandai angka kunci dalam soal, lalu terapkan operasi yang tepat.',steps:[q.hint||'Kenali angka yang menjadi dasar perhitungan.','Hitung dengan teliti, jangan terburu-buru.',`Jawabannya: ${val}.`],insight:'Baca soal perlahan dan tandai angka kuncinya dulu.',visual:{type:'formula',values:[],labels:['Jawaban',val]},source:'lokal'}; }
